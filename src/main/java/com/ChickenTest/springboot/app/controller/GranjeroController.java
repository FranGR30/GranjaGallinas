...
public class GranjeroController {
	...
	// Parametros: id granjero, cantidad de gallinas en el contador de la tienda,
	// cantidad de huevos en el contador de la tienda, el tipo de objeto a comprar
	// (0=gallinna, 1=huevo)
	...
	public String comprar(@PathVariable(value = "id") Long id,
		...
		if (tipo == 0) {// En caso de que el tipo sea = 0 procede a agregar gallinas
			if ((granjero.getGallinas().size() + cantGallinaTienda) > granjero.cantGallinasMax) {// En caso se superar
																									// la cantidad de
																									// gallinas maximas
																									// en inventario,
																									// arroja error
				...
				crearObjetoIterable(cantGallinaTienda, granjero, tipo);// La funcion crea la cantidad de gallinas a
																		// comprar por el usuario y las asigna al
																		// granjero
				granjero.setDinero(granjero.getDinero() - (granjero.precioGallinaCompra * cantGallinaTienda));// Se
																												// descuenta
																												// el
																												// dinero
																												// del
			...
			if ((granjero.getHuevos().size() + cantHuevoTienda) > granjero.cantHuevosMax) {// En caso se superar la
																							// cantidad de huevos
																							// maximos en inventario,
																							// arroja error
				...
				crearObjetoIterable(cantHuevoTienda, granjero, tipo);// La funcion crea la cantidad de huevos a comprar
																		// por el usuario y los asigna al granjero
				granjero.setDinero(granjero.getDinero() - (cantHuevoTienda * granjero.precioHuevoCompra));// Se
																											// descuenta
																											// el dinero
																											// del
																											// granjero
	...
	public String vender(@PathVariable(value = "id") Long id,
		...
		if (tipo == 0) {// Valida si es gallina
			if (granjero.getGallinas().size() >= cantGallinaTienda) {// Valida si tiene la cantidad de producto que se
																		// quiere vender
				eliminarObjetoIterable(cantGallinaTienda, granjero, tipo);// Elimina la cantidad a vender seleccionada
																			// por el usuario
			...
			if (granjero.getHuevos().size() >= cantHuevoTienda) {
				eliminarObjetoIterable(cantHuevoTienda, granjero, tipo);// Elimina la cantidad a vender seleccionada por
																		// el usuario
	...
	// Funcion para pasar al dia siguiente poniendo huevos, naciendo gallinas o
	// muriendo gallinas
	...
	public String diaSiguiente(@PathVariable(value = "id") Long id, Model model) {
		...
		if (!granjero.getGallinas().isEmpty()) {
			for (int i = 0; i < granjero.getGallinas().size(); i++) {// Itera segun cuantas gallinas tenga el granjero
				...
				if (gallina.getDiasDeVida() >= gallina.getDiaMuerte()) {// Valida si los dias de vida de la gallina son
																		// iguales al dia de muerte de la gallina
																		// (generado de forma aleatoria)
					granjero.getGallinas().remove(i);
					contGalinaRemove++;
				}
				Random numAleatorio = new Random();// Genera un numero random entre 0 y 1. De esta forma, cada gallina
													// tiene un 50% de chance de poner un huevo cada dia hasta que su
													// contador de huevos a poner llegue a cero
				...
				if (ponerHuevo == 1) {// En caso de que el numero random es 1, pone un huevo
					if (gallina.getHuevosAPoner() > 0) {
						if ((granjero.getHuevos().size() + 1) <= granjero.cantHuevosMax) {// Valida que no se pase del
																							// maximo de huevos posibles
																							// para el granjero
		...
		if (!granjero.getHuevos().isEmpty()) {
			for (int j = 0; j < granjero.getHuevos().size(); j++) {// Itera segun la cantidad de huevos tenga el
																	// granjero
				...
				if (huevo.getDiaNacimiento() <= huevo.getDiasDeVida()) {// Verifica si el huevo tiene dias de vida
																		// suficientes para nacer
	...
	// Parametros: id granjero, contador de gallina, contador de huevo, el tipo de
	// producto que se quiera sumar(gallina o huevo)
	...
	// Parametros: id granjero, contador de gallina, contador de huevo, el tipo de
	// producto que se quiera restar(gallina o huevo)
